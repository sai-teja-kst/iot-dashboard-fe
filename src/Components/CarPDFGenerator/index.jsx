...
export const CarPDFGenerator = ({ pdftemplate, vehicledata }) => {
  ...
  return (
    <Col xs={12}>
      <Container id="pdf" className={`border d-none border-secondary ${pdftemplate}`}>
        <Row>
          <Col xs={12} md={6}>
            {vehicledata && (
              <Card className="p-3 car-card mb-3">
                ...
                <div className="mb-3">
                  {vehicledata.status === "done" && (
                    <h5 className="text-success fw-bold">✅ Approved</h5>
                  )}
                  {vehicledata.status === "inprogress" && (
                    <h5 className="text-warning fw-bold">⏳ In Progress</h5>
                  )}
                  {vehicledata.status !== "done" &&
                    vehicledata.status !== "inprogress" && (
                      <h5 className="text-secondary fw-bold">🕒 To Do</h5>
                    )}
                </div>
                  ...
                  <>
                    ...
                    <div className="d-flex flex-column gap-2 ps-1">
                      {Object.entries(vehicledata.checklist).map(
                        ([section, result]) => (
                          <div key={section} className="form-check d-flex align-items-center">
                            <input
                              ...
                              readOnly
                              style={{
                                borderColor: result ? "#000" : "#ccc",
                                backgroundColor: result ? "#000" : "#fff",
                              }}
                            />
